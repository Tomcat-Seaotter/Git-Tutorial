...
GitHub的官方网址：[https://github.com]，如果没有账号，可以注册一个。点击Sign in进入登录界面，输入账号和密码登入GitHub。下面是本人的GitHub：[https://github.com/nb-snowpiercer]
...
免安装版本，将压缩文件解压后，点击post-install.bat文件。

## 2-3 绑定用户

在桌面快捷方式/开始菜单/安装目录中打开`git-bash.exe`，Git是分布式版本控制系统，所以需要填写用户名和邮箱作为一个标识，用户和邮箱为你GitHub注册的账号和邮箱。
...
生成成功后，去对应目录C:\Users\Y\ .ssh里（Y为电脑用户名，每个人不同）用记事本、Sublime text打开id_rsa.pub，得到ssh key公钥。
...
git push -f origin master          //把本地库的所有内容强制推送到远程库上
```

或者可以直接通过命令行push一个存在的代码库。

```shell
git remote add origin git@github.com:Tomcat-Seaotter/git-test.git
git branch -M main
git push -u origin main
```

或者从其他代码库导入代码。你可以从Subversion, Mercurial, TFS project初始化代码库。
...
```
 ...
 git add . ，注意 "." 前面有空格，添加所有的文件， 或者 git add --all 添加所有的文件
